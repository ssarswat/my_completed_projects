...
SELECT 	
        COUNT(id) AS number_of_movies_produced_in_2019
FROM
		movie
WHERE
...
SELECT 
    genre, COUNT(movie_id) AS number_of_movies_produced
FROM
    genre AS g
        INNER JOIN
...
(The comment above the problem statement instructs to find the highest number of movies produced in last year i.e. 2019 
whereas the problem statement says produced overall. In case of last year, WHERE statement has to be added to the code.) 
*/    
GROUP BY genre
...
WITH count_of_movie_genre AS

(
	SELECT 
		movie_id,
		COUNT(genre) AS number_of_genre_of_movie
	FROM
		genre
	GROUP BY
		movie_id
	HAVING
		number_of_genre_of_movie=1
)

SELECT 
	COUNT(movie_id) AS number_of_movies_with_one_genre
FROM
	count_of_movie_genre;
...
-- Took 2 Decimal places because next comment hint mentioned average duration of Drama genre movies as 106.77
...
-- Thriller genre is ranked 3 amongst all genres with respect to movie count in each genre
...
WHERE
	country like '%USA%' AND MONTH(date_published)=3 AND year=2017 AND total_votes>1000
GROUP BY
	genre
ORDER BY
	movie_count DESC;
...
SELECT 
	COUNT(id) as movies_between_01Apr18_till_01Apr19_median_rating_8
FROM 
	movie as m
INNER JOIN
	ratings as r
...
top_3_genre
WHERE
	g.genre IN (top_3_genre.genre) AND avg_rating>8
GROUP BY 
	director_name
ORDER BY 
	movie_count DESC
...
/* The rank of the directors is as follows:
     director_name          director_rank
1.  James Mangold                1
2.  Soubin Shahir                2
3.  Joe Russo                    2
4.  Anthony Russo                2

There are 3 directors having director_rank '2'.
However, while limiting the results only director_rank '1' and 2 directors out of 3 having director_rank '2' are being displayed.
*/
...
ON 
	m.id= rm.movie_id
WHERE 
	m.country LIKE 'India' AND rm.category LIKE 'actor'
GROUP BY
	actor_name
HAVING
	movie_count>=5;
...
ON
	m.id=r.movie_id
WHERE
	median_rating>=8 AND languages LIKE '%,%' AND production_company IS NOT NULL
GROUP BY
	production_company
LIMIT 2;
